...
FileProcessor.prototype.process = function(filename, assetSearchPath, replaceAssets) {
  ...
  //First phase of usemin: ReplaceBlocks
  var content = this.replaceBlocks(this.file);

  //Second phase of usemin: Replace With Revved
  if(replaceAssets){
    if (assetSearchPath && assetSearchPath.length !== 0) {
      this.file.searchPath = assetSearchPath;
    }
    content = this.replaceWithRevved(content, this.file.searchPath);
  }
